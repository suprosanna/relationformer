...
def bboxes_copy_paste(bboxes, paste_bboxes, masks, paste_masks, alpha, key):
    if key == 'paste_bboxes':
        ...
        adjusted_paste_bboxes = [apbox + tail[4:] for apbox, tail in
                                 zip(adjusted_paste_bboxes, paste_bboxes)]
...
def copy_paste_class(dataset_class):
    def _split_transforms(self):
        ...
        if split_index is not None:
            ...
            # recreate transforms
            self.transforms = A.Compose(pre_copy, bbox_params, keypoint_params,
                                        additional_targets=None)
            self.post_transforms = A.Compose(post_copy, bbox_params, keypoint_params,
                                             additional_targets=None)
            self.copy_paste = A.Compose(
                [copy_paste], bbox_params, keypoint_params,
    ...
    def __getitem__(self, idx):
        ...
        img_data, target = self.load_example(idx)
        if self.copy_paste is not None:#train
            if self.cluster_copy_paste:
                paste_idx = random.choice(self.cluster_dict[self.id_dict[self.img_id]]).item()
            else:
                paste_idx = random.randint(0, self.__len__() - 1)
            paste_img_data, _ = self.load_example(paste_idx, self.cluster_copy_paste)
        ...
        target['labels'] = torch.tensor([img_data['bboxes'][i][4] for i in range(len(img_data['bboxes']))], dtype=torch.int64)
        target['boxes'] = torch.tensor(img_data['bboxes'], dtype=torch.float32)[:, :-2]
        h, w = target['orig_size']
        target['area'] = []
        for idx, bboxes in enumerate(target['boxes']):
            target['area'].append(w * (bboxes[2] - bboxes[0]) * h * (bboxes[3] - bboxes[1]))
            target['boxes'][idx] = box_ops.box_xyxy_to_cxcywh(bboxes)
        target['area'] = torch.tensor(target['area'], dtype=torch.float32)
        target.pop('masks')
        #TODO: target iscrowd expand
        return image, target
