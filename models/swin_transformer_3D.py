...
class PatchEmbed3D(nn.Module):
    ...
    """
    def __init__(self, img_size=(64,64,64), patch_size=(4,4,4), in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            x = x.flatten(2).flatten(2).transpose(1, 2)
            x = self.norm(x)
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.return_layers = [self.num_layers-3, self.num_layers-2]
    ...
    def forward(self, x, position_embedding, return_interm_layers=False):
        """Forward function."""
        ...
        for i,layer in enumerate(self.layers):
            ...
            if i in self.return_layers and return_interm_layers:
                feat_list.append(x)
                mask = torch.zeros(x[:,0,...].shape, dtype=torch.bool).cuda()
                mask_list.append(mask)
                pos_embed_list.append(position_embedding(mask).cuda())
        ...
        mask_list.append(mask)
        pos_embed_list.append(position_embedding(mask).cuda())
        return feat_list, mask_list, pos_embed_list
...
def build_swin_transformer(config):
    model = SwinTransformer3D(img_size=config.DATA.IMG_SIZE,
                            patch_size=config.MODEL.ENCODER.PATCH_SIZE,
                            in_chans=config.MODEL.ENCODER.IN_CHANS,
                            num_classes=config.MODEL.NUM_CLASSES,
                            embed_dim=config.MODEL.ENCODER.EMBED_DIM,
                            depths=config.MODEL.ENCODER.DEPTHS,
                            num_heads=config.MODEL.ENCODER.NUM_HEADS,
                            window_size=config.MODEL.ENCODER.WINDOW_SIZE,
                            mlp_ratio=config.MODEL.ENCODER.MLP_RATIO,
                            qkv_bias=config.MODEL.ENCODER.QKV_BIAS,
                            qk_scale=config.MODEL.ENCODER.QK_SCALE,
                            drop_rate=config.MODEL.DROP_RATE,
                            drop_path_rate=config.MODEL.DROP_PATH_RATE,
                            ape=config.MODEL.ENCODER.APE,
                            patch_norm=config.MODEL.ENCODER.PATCH_NORM,
